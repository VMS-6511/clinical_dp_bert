...
def run_customized_training_loop(
  ...
  with distribution_utils.get_strategy_scope(strategy):
    ...
    # tvars = [v for v in training_vars if 'bert' not in v.name]
    def _replicated_step(inputs):
      ...
      if explicit_allreduce:
        ...
        if isinstance(optimizer,
          ...
          grads, vars_tf = optimizer.compute_gradients(loss, training_vars)
          grads_and_vars = list(zip(grads, vars_tf))
        optimizer.apply_gradients(iter(grads_and_vars))
      # For reporting, the metric takes the mean of losses.
      train_loss_metric.update_state(raw_loss)
      for metric in train_metrics:
        metric.update_state(labels, tf.nn.softmax(model_outputs,axis=1)[:,1])
    ...
    if eval_metrics:
      ...
      training_summary['last_train_metrics'] = _float_metric_value(
          train_metrics[0])
      training_summary['eval_AUC'] = _float_metric_value(eval_metrics[0])
