...
from tensorflow_privacy.privacy.optimizers import dp_optimizer_bert
...
flags.DEFINE_float('l2_norm_clip',1.0,'DP L2 Clip')
flags.DEFINE_float('noise_multiplier',1.0,'DP Noise Multiplier')
flags.DEFINE_string('results_dir',None,'Dir to save results')
...
def get_loss_fn(num_classes):
  ...
  def classification_loss_fn(labels, logits):
    """Classification loss."""
    if FLAGS.optimizer_type != 'dpadamw':
...
def run_bert_classifier(strategy,
  ...
  def _get_classifier_model():
    ...
    optimizer = optimization.create_optimizer(
        ...
        FLAGS.optimizer_type, l2_norm_clip=FLAGS.l2_norm_clip, noise_multiplier=FLAGS.noise_multiplier)
  ...
  def metric_fn():
    return tf.keras.metrics.AUC(
        name='test_AUC', dtype=tf.float32)
  ...
  logging.info('Training using customized training loop TF 2.0 with '
               'distribution strategy.')
  logging.info(FLAGS.results_dir)
