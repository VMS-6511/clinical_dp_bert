    ...
    def compute_gradients(self,
      ...
      if callable(loss):
        ...
        def process_microbatch(i, sample_state):
          """Process one microbatch (record) with privacy helper."""
          def new_loss_fn():
            return tf.reduce_mean(input_tensor=tf.gather(
                  microbatches_losses, [i]))
          grads, _ = zip(*super(cls, self)._compute_gradients(
              new_loss_fn, var_list, grad_loss))
          grads_list = [
        ...
        final_grads = tf.nest.map_structure(normalize, grad_sums)
        grad_norm = tf.linalg.global_norm(final_grads)
        return final_grads, var_list
